...
export async function buildServerInstructions(
  ...
  const sections: string[] = [
    ...
    `DEPENDENCIES: before changing something, get_dependents with a term (pricing/tier-2) or a slug to see which pages depend on it, which page asserts it (source of truth), which pages were built from it as a template, and which external assets (a Drive deck, a GitHub README) hang off it. Every row has verifiedAt and staleAgainstSource: true means the source changed after that row was last looked at. To build a graph, call map_dependencies once with the term, the asserts/depends/references page slugs, and any external urls. After reviewing a dependent, mark_verified it with the term you checked it against (slug or url plus term); status holds if it is fine, needs_change if it is wrong, note for what you found. Verification is per page-concept edge, so checking a page for pricing says nothing about the tagline. Writing a page verifies all its edges automatically. A map can reuse another map's checklist with includes (map_dependencies term=<parent> includes=[<child>]), for a recurring group like sales-enablement; checking a shared row for one parent never marks it checked for another parent that also includes it, or for the child's own view - pass context=<parent term> on mark_verified when verifying a row you can see is tagged group in get_dependents. Humans see the same graph at /map (every concept, ranked by what still needs a look) and /map/<term>; link them there rather than pasting the JSON. Pages created with create_from_template are tagged template/<template-slug> automatically, so search_pages "template/<slug>" lists every instance.`,
