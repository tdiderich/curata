import { db } from "./db";
import { instructionsSection } from "@/lib/mcp-guidance";
...
export async function buildServerInstructions(
  ...
  const sections: string[] = [
    ...
    `TAGGING: tag every page you write with concepts - untagged pages are invisible in the brain map. Prefer the canonical tags where they fit (${DEFAULT_TAGS.join(", ")}) plus any specific concepts; combine tags freely - engineering + go-to-market marks something engineers should know that customers will ask about. Reuse existing tags from the brain map over inventing near-synonyms. Each tag can carry a rel: depends (this page is wrong if the concept changes), asserts (this page is the source of truth for it), or references (default, a plain mention). Namespace terms with one slash where it helps, like feature/gcp-support or pricing/tier-2.`,
    `CHART: curata draws an content map from what already exists: template instances, pages embedding another page's components via a ref block, pages tagged depends on a concept some page asserts, and external assets put under a node. Anything with three or more children is a node. get_chart lists nodes with one color each: green (children checked since the source last moved), yellow (source moved since last check, or never checked, or a mismatch note), red (a mismatch found via needs_change, or yellow plus past due, or the source moved twice since the last check). get_chart term=<node> shows the children with reasons, suggested external reports, and pageSuggestions: pages that mention the node by name but are not under it, read those and add_to_chart slug= the ones that depend on it; get_needs_look is the yellow/red list grouped by node. Before changing a source page, get_chart term= or get_dependents to see what goes yellow; after, the write response's impact field says how many did. To see what a source change actually was, read_version slug= version_id=latest compare_to=previous gives the line diff. Clear a page child by rewriting the page or mark_verified slug= term=. For externals, call audit: withRecipe items carry check {via, tool, locator, ask}, run it through that MCP in your session, compare to the source, then mark_verified url= term= status=holds, or status=needs_change note=<what's off>, or status=unreachable note=<why> when you have no access to that system; humanOnly items go to the human. Write responses carry trust: if trustedBehind is true the page is trust-locked and readers still see the pinned version until a human runs mark_trusted, say so. add_to_chart puts a page or url under a node (get_chart's suggestions first); set_scope_item sets owner, due_at and check on an external. Checks are per edge: a deck under pricing and under sso is two checks, alsoUnder tells you which others still need one. Humans see the same chart at /map and /map/<term>; link them there rather than pasting JSON. Pages created with create_from_template are tagged template/<template-slug> automatically.`,
    ...
    `DIGEST: generate_digest reports One big thing (a human-picked headline), Noteworthy (2-3 short items), and Activity (stats plus hot spots) since the last digest run (or the last 7 days, the first time). Call with preview: true first to see the window's candidate pages and counts without writing anything, read a few candidates, then gate on a human pick before calling again with big_thing/noteworthy to write the page - see the curata-digest skill for the full flow. Safe to re-run: a second call in the same week updates that week's page instead of creating a duplicate. Digest pages are system reports: read them by slug or on the latest channel, they never need trust approval (trusted stays reserved for human-approved claims).`,
    `REVIEW: get_review_queue lists pages awaiting human trust review, rules-scoped (a never-trusted page only queues where an approval rule applies to its scope, a trustedBehind page always queues, a locked folder never queues). When asked to pre-screen the queue: for each item read_page on channel "trusted" and channel "latest" to diff, list_rules to check the folder's rules, search_pages plus get_related to check for likely duplicates, validate_page for structural problems, then annotate_page one finding per issue (kind "note", source "prescreen"). Never call mark_trusted from a pre-screen, findings are advisory only.`,
    instructionsSection().trim(),
    `SHARED COMPONENTS: a page with pageType: component holds a small components array meant to be reused, not read on its own — a roadmap snippet, a pricing table, a support contact list. Embed one into any other page with a \`type: ref\` block (\`- type: ref\` / \`component: <slug>\`); it expands to the source page's content at read time, never stored expanded. Approving a new version of the source page (mark_trusted) fans out to every page that embeds it with no further edits. To change what a ref shows, patch_page or write_page the source page's own slug — a consuming page's stored content is just the ref block, so there's nothing else on it to edit. Make a component page only when the same block needs to update in more than one place at once; a page nobody else embeds should stay a normal page. list_templates/create_from_template ship a handful of starter component pages (roadmap, pricing, team roster, changelog...) to copy from.`,
  ];
