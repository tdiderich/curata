...
import { createNode, getChart, getChartNode, getNeedsLook, promotePageToNode, setChartNode } from "@/lib/chart";

/** GET ?term= for one node, ?view=list for needs-look, else the whole chart. PATCH {term, hidden|promoted}. */
...
export async function PATCH(request: NextRequest) {
  ...
  let body: { term?: string; hidden?: boolean; promoted?: boolean };
  ...
  if (Object.keys(patch).length === 0) return NextResponse.json({ error: "give at least one of hidden, promoted" }, { status: 400 });
  try {
