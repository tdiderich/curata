...
// as HTML. The gate lives in resolvePublicPage, shared with /md, so the
// non-HTML representations cannot drift apart: the page must be
...
export async function GET(request: Request, { params }: Ctx) {
  ...
  const data = await readPageYaml(ref.orgId, pageSlug);
